...
%
% Функция ChangeVariable преобразует систему линейных отношений
...
% плоскости A(i,:) x = b(i).
% (Компонентами вектора relations служат "=", "<", ">", "<=", ">=".
% Вектор relations* получается из relations удалением i-й компоненты).
%
...
% дата:  15/09/2012
   ...
   % выбор внутренних ортов y1 и y2
     ...
     [c,l]=min(abs(A(i,:))); % в координатной плоскости ~l 
                             % проекция вектора A(i,:) наиболее длинная
     l1=mod(l,3)+1;
     l2=mod(l1,3)+1;
     y1(l1)=A(i,l2);
     y1(l2)=-A(i,l1);
